...
app.config['SECRET_KEY'] = 'uma_chave_secreta_muito_segura_e_dificil_de_adivinhar_123!@#'
...
def load_user(user_id):
    ...
    if user_data:
        return User(id=user_data['id'], username=user_data['username'], full_name=user_data['full_name'], 
                    password_hash=user_data['password_hash'], is_admin=bool(user_data['is_admin']))
    return None
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Ocorreu um erro inesperado ao tentar realizar o cadastro.', 'error')
            db.rollback()
        return redirect(url_for('register'))
...
def add_donor():
    ...
    try:
        cursor = db.cursor()
        cursor.execute(
            """INSERT INTO donors (name, birth_date, weight, blood_type, last_donation_date, contact_info,
...
def get_donors():
    ...
    for donor in donors_list:
        ...
        if donor['calculated_next_donation_date']:
            try:
                next_date_obj = date.fromisoformat(donor['calculated_next_donation_date'])
...
def generate_spreadsheet():
    ...
    headers = [
        ...
        "Email/Telefone", "Última Doação", "Status Triagem", "Dias Inapto", 
        "Mensagem Triagem", "Próxima Data Possível", "Data Cadastro"
    ]
    ...
    for donor in donors_data:
        ...
        row_data = [
            ...
            contact, last_d, donor['triage_result_status'], 
...
# ======================================================================
#     NOVA ROTA PARA DELETAR UM DOADOR
# ======================================================================
...
def delete_donor(donor_id):
    ...
    db = get_db()
    donor = db.execute('SELECT id FROM donors WHERE id = ?', (donor_id,)).fetchone()
    ...
    try:
        ...
        db.commit()
        return jsonify({'message': 'Doador removido com sucesso!'}), 200
    except Exception as e:
        db.rollback()
        current_app.logger.error(f"Erro ao deletar doador: {e}")
        return jsonify({'error': 'Erro interno do servidor ao tentar remover doador.'}), 500
# ======================================================================
